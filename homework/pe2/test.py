...
def assemble_stokes(
    ...
    dof = 2 * N_u + N_p
    M = lil_matrix((dof, dof))

    g: NDArray[np.floating] = np.zeros(dof)
    ...
    for e in range(num_elements):
        ...
        for i_func in range(p_2_n):
            for j_func in range(p_2_n):
                ...
                for i_quad in range(N_quadr):
                    dphi_i = dshape2_ref[i_quad, i_func] @ invJ
                    dphi_j = dshape2_ref[i_quad, j_func] @ invJ
        ...
        for i in range(p_2_n):
            for j in range(p_2_n):
                M[con6[i], con6[j]] += elemA[i, j]
                M[con6[i] + N_u, con6[j] + N_u] += elemA[i, j]
        ...
        for i_func in range(p_2_n):  # 6
            for j_func in range(p_1_n):  # 3
                tempx = 0
                tempy = 0
                for i_quad in range(N_quadr):
                    ...
                    dphi2 = dshape2_ref[i_quad, i_func] @ invJ
                    x_dphi2 = dphi2[0]
                    y_dphi2 = dphi2[1]
        ...
        for i in range(p_2_n):
            for j in range(p_1_n):
                p_coords = 2 * N_u + con3_mapped[j]
                M[con6[i], p_coords] += elemBx[i, j]
                M[con6[i] + N_u, p_coords] += elemBy[i, j]

                M[p_coords, con6[i]] += elemBx[i, j]
                M[p_coords, con6[i] + N_u] += elemBy[i, j]
